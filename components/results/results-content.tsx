...
import { type EvalResult, type RiskLevel, type NewsArticle, type IntakeData } from '@/types';
...
function parseRiskScore(text: string) {
  ...
  const rationale = rationaleMatch
    ? rationaleMatch[1].replace(/[-*]/g, '').replace(/\s+/g, ' ').trim()
    : '';
...
function parseTamStats(text: string): { tam: string | null; cagr: string | null; year: string | null } {
  ...
  const tam = rangeMatch
    ? `$${rangeMatch[1]}–${rangeMatch[2]}B`
    : singleMatch
      ? `$${singleMatch[1]}B`
      : null;
  const cagr = text.match(/(\d+\.?\d*)%\s*(?:CAGR|cagr)/)?.[1]
    ? `${text.match(/(\d+\.?\d*)%\s*(?:CAGR|cagr)/)![1]}%`
    : null;
...
export function ResultsContent({ result, intake, generatedAt, sessionId }: ResultsContentProps) {
      ...
      <div className="top-0 z-10 bg-white -mx-7 px-7 pt-4 pb-3 border-b border-slate-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-slate-900">Validation Memo: {intake.company}</h1>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-6 flex flex-col justify-center items-center gap-3">
            ...
            {justification && <p className="text-xs text-slate-500 text-center leading-relaxed">{justification}</p>}
          </div>
          ...
          {newsArticles.length > 0 ? (
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide w-8">
                    #
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">
                    Source
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide w-28">
                    Date
                  </th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wide w-16">
                    Summary
                  </th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wide w-16">
                    Link
