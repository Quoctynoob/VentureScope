...
import { saveIntakeData } from '@/lib/actions/intake';
import { upsertSession } from '@/lib/actions/sessions';
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full min-h-[60vh] px-12">
      ...
      <p className="text-sm text-slate-400 text-center max-w-md mb-8">
        Querying You.com APIs for real-time web data, validating structured signals, and generating
...
interface ReviewFormProps {
  initialData: IntakeFormValues;
}

export function ReviewForm({ initialData }: ReviewFormProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [data, setData] = useState<IntakeFormValues>(initialData);
  ...
  function removeCompetitor(name: string) {
    set(
      'knownCompetitors',
      (draft.knownCompetitors ?? []).filter((c) => c !== name)
    );
  }
  ...
  async function handleFinalSubmit() {
    ...
    try {
      ...
      const confMatch = (result.riskScore?.text as string | undefined)?.match(
        /Confidence Score[^:*]*[:\*]+\s*\*?\*?(\d+)%/i
      );
      const confidence = confMatch ? parseInt(confMatch[1]) : 75;
      const riskMatch = (result.riskScore?.text as string | undefined)?.match(
      ...
      // Save to server action instead of localStorage
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Check each section and edit if needed before submitting.
        </p>
      </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  value={draft.primaryGeography}
                  onValueChange={(v) => set('primaryGeography', v)}
                >
                  <SelectTrigger className="w-full">
                    <SelectValue />
                  </SelectTrigger>
        ...
        {isEditing(3) ? (
          <div className="space-y-5">
            <div>
              ...
              <Select
                value={draft.revenueModelStructure}
                onValueChange={(v) => set('revenueModelStructure', v)}
              >
                <SelectTrigger className="w-full">
                  <SelectValue />
                </SelectTrigger>
              ...
              {data.knownCompetitors?.length ? (
                <div className="flex flex-wrap gap-2 mt-2">
                  {data.knownCompetitors.map((name) => (
                    <span
                      key={name}
                      className="bg-slate-100 text-slate-700 text-sm font-medium px-3 py-1 rounded-sm"
                    >
                      {name}
                    </span>
                  ))}
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    value={draft.monthlyRecurringRevenue ?? ''}
                    onChange={(e) => set('monthlyRecurringRevenue', e.target.value)}
                ...
                <div>
                  ...
                  <Input
                    value={draft.activeCustomerCount ?? ''}
                    onChange={(e) => set('activeCustomerCount', e.target.value)}
                    placeholder="e.g. 42"
                  />
                </div>
              </div>
              <div>
                ...
                <Input
                  value={draft.monthOverMonthGrowth ?? ''}
                  onChange={(e) => set('monthOverMonthGrowth', e.target.value)}
                  placeholder="e.g. 18%"
                />
              </div>
            </div>
      ...
      <div className="bg-white rounded-l border border-slate-200 shadow-sm p-8">
        <div className="flex items-start gap-3">
          <div
            className={`mt-0.5 h-4 w-4 rounded border flex items-center justify-center shrink-0 ${data.evaluationTerms ? 'bg-blue-600 border-blue-600' : 'border-slate-300'}`}
          >
            {data.evaluationTerms && <Check className="w-3 h-3 text-white" />}
          </div>
          <div>
            <p className="text-sm font-semibold text-slate-800">
              Evaluation Terms & Confidentiality Confirmation
            </p>
            <p className="mt-1 text-sm text-slate-400">
              Founder accepts non-confidential evaluation terms and acknowledges that synthesis
              results are for internal assessment only.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-2">
        <Button
          size="sm"
          variant="outline"
          className="rounded-sm border-blue-500 border text-blue-500"
